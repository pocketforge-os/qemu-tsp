// mkuinput.c — host-native uinput virtual gamepad creator. Stays alive (pause()).
// On SIGUSR1, injects a small known event sequence so a reader can be tested.
...
static void inject(int sig){
  (void)sig;
  // Known sequence: BTN_SOUTH press + SYN, ABS_Z=128 + SYN, BTN_SOUTH release + SYN
...
int main(){
  ...
  int abss[]={ABS_X,ABS_Y,ABS_Z,ABS_RZ,ABS_HAT0X,ABS_HAT0Y};
